...
import { GENIE_GLOBAL_KEY } from '@genie-react/core'
...
async function loadGenie(router: unknown) {
  ...
  vi.doMock('@genie-react/client', () => ({
    createGenieClient,
    sessionCollector: tag('session'),
  }))
  vi.doMock('@genie-react/react-collector', () => ({ reactCollector: tag('react') }))
  vi.doMock('@genie-react/memory', () => ({ memoryCollector: tag('memory') }))
  vi.doMock('@genie-react/devtools-plugin', () => ({ pluginPassthroughCollector: tag('plugin') }))
  vi.doMock('@genie-react/tanstack-collector', () => ({
