...
This example uses `apps/plain-demo` from a local checkout of this repository. Run `pnpm install`
from the repository root first. You also need Bash, `jq`, and a working `agent-browser` installation.
Run every block from the repository root, and keep each long-running process in its own terminal.
...
status_json="$(genie status --sessions-only)"
...
renders_json="$(genie call react_get_renders \
  '{"component":"EffectCounter","sort":"selfTime","limit":3}')"
jq '{
...
causes_json="$(genie call react_render_causes \
  '{"component":"EffectCounter","limit":3}')"
...
agent-browser --session "$GENIE_SESSION" get text '[data-testid="increment"]'
genie call react_clear_renders '{}' >/dev/null
