...
Desktop React Native targets (react-native-macos, react-native-windows) are the easy case: the app
and the hub run on the same machine, so `ws://127.0.0.1:4390/__genie/ws` works with no port
forwarding.
