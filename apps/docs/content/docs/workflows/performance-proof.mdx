...
Genie's active render analysis adds synchronous work to development commits. `react_profile_stop`
pauses detailed collection while the hook remains installed. Keep collector settings and coverage
equivalent across both cohorts. For an overhead report, also measure the same flow with Genie
disabled; a faster instrumented run alone does not measure the cost seen by an uninstrumented app.

The repository's [runtime overhead report](https://github.com/Genie-sa/genie-react/blob/main/docs/runtime-overhead.md)
documents a 500-row development fixture and its limits. Its timings do not establish overhead in
your application or in production.
...
A recorded Router demo run produced the rejection below. These selected fields show how to read
an incomplete comparison; your current run may have different coverage:
...
Fix the coverage gap or narrow the claim, then repeat both cohorts.
`pass` means Genie found no budget violation. To claim an improvement, also require
...
Return the comparison verdict, usable sample counts, rejected evidence, budget results, and the
visible behavior check. Claim a performance improvement only for metrics that meet the comparison
criteria above.
