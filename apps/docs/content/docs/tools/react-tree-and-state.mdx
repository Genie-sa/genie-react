...
**Output (selected fields):** `status.system` (`stylex` or `none`) with a `hint` naming the plugin
option that unlocks more; per element `styleObjects[]` (`name`, `package`, `file`, `line` of the
style key), `declarations[]` (`property`, `value`, `condition` such as `:hover` or `@media (...)`,
`tokens[]` with current variable values), `dynamic[]`, and `unresolvedClasses[]`.
...
**Agent use:** The last entry in `applied` owns each property it declares, so edit that definition.
Values are declared CSS; measure rendered geometry with a browser tool.
