...
Use `status --verbose` for connection diagnostics; `doctor` accepts `--live` for stack probes.
...
Check [CLI release compatibility](/docs/getting-started/cli-output) for older versions that emit text.
