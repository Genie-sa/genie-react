...
Use the UI driver to check visible state and Genie to inspect React causes and Query observer
delivery. This case records an Expo Go run on an iPhone simulator; it does not establish behavior
on every device or React Native version.
...
Check these selected status fields before continuing:
...
timing, the [timeline workflow](/docs/workflows/interaction-timeline) explains native lane limits.
