...
The recorded Router demo comparison collected all 12 captures. Genie removed one warm-up from
...
The captures existed, but their render coverage could not support an optimization claim.
Fix the coverage gap or report only the metrics whose evidence is comparable.
