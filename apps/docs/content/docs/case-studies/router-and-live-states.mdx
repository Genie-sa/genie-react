...
Selected fields from `router_get_state`:

```json
{
  "pathname": "/suspense",
  "searchStr": "?source=docs",
  "status": "idle",
  "matchCount": 2,
...
The browser text check returned `Lazy content loaded`.
...
Require an entry in `react_error_state.suspended` with `forced: true` and
`isFallbackShowing: true`. The browser text must read `Loading lazy panel…`.
...
Require an entry in `react_error_state.caughtErrors` with `forced: true` and
`boundaryName: "Boundary"`. The browser must show the simulated error and its reset control.
